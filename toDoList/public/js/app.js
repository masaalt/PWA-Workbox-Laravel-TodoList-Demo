...
function createIndexedDB() {
  var dbPromise = (0,idb__WEBPACK_IMPORTED_MODULE_0__.openDB)('dashboardr', 1, {
...
var dbPromise; // if(navigator.onLine){
...
function getServerData() {
  return fetch('/getdata').then(function (response) {
    if (!response.ok) {
...
function loadContentNetworkFirst() {
  getServerData().then(function (dataFromNetwork) {
    updateUI(dataFromNetwork);
    saveEventDataLocally(dataFromNetwork).then(function () {
    ...
    getLocalEventData().then(function (offlineData) {
...
function messageSaveError(err) {// alert user that data couldn't be saved offline
  // alert("error:"+err);
}

function messageDataSaved() {
  // alert user that data has been saved for offline
  var lastUpdated = getLastUpdated(); // if (lastUpdated) {dataSavedMessage.textContent += ' on ' + lastUpdated;}
...
function messageOffline() {
  ...
  if (lastUpdated) {// alert(' Last fetched server data: ' + lastUpdated) ;
  } // offlineMessage.style.display = 'block';

}

function messageNoData() {// alert user that there is no data available
  // alert(' No data! ') ;
}
